...
window.Clicked = function (x) {
    ...
    if (clicked.length === 0) {
        ...
        // highlight chai có thể đổ
        for (let i = 0; i < water.length; i++) {
            if (canMove(water, x, i)) {
                document.getElementById(`tube-${i}`).classList.add("highlight");
            }
        }
    } else {
        // --- CHỌN CHAI THỨ HAI ---
...
function isVictory(state) {
    return state.every(t => {
        let first = t[0];
        if (first === "transparent") return true; // cho phép chai rỗng
...
window.Undo = function () {
    if (history.length > 0 && !transferring) {
        water = history.pop();
        if (moves > 0) moves--;
